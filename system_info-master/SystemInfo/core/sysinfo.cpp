...
unsigned int getCpuUsagePercentageEx() {
...
unsigned int getCpuUsagePercentage() {
	int logicCores = numberOfLogicalProcessors();
	vector<LPCWSTR> queryAttrs = wmiClassStringsMap.at(L"Win32_PerfFormattedData_PerfOS_Processor");
	//queryAttrs.push_back(L"Name");
	//queryAttrs.push_back(L"PercentUserTime");
	//queryAttrs.push_back(L"PercentPrivilegedTime"); //ThreadCount 
	queryAttrs.push_back(L"ThreadCount");
	IEnumWbemClassObject* pEnumerator = executeWQLQuery
	(wmiWbemInfo->getHres(), 
		wmiWbemInfo->getWbemLocator(), 
		wmiWbemInfo->getWbemServices(), 
		//buildQueryString(L"Win32_PerfFormattedData_PerfOS_Processor", queryAttrs, L"Name='_Total'"));
		//buildQueryString(L"Win32_PerfFormattedData_PerfOS_Processor", queryAttrs, L"Name!='_Total'"));
		buildQueryString(L"Win32_PerfFormattedData_PerfProc_Process", queryAttrs, L"Name!='_Total'"));
	IWbemClassObject* pclsObj = NULL;
	ULONG uReturn = 0;
	//if -1, then WMI for some reason didn't get the data from the tables
	int cpuUsage =  0;
	...
	std::string processStr = "";
		while (pEnumerator) {
			...
			///
			hr = pclsObj->Get(queryAttrs.at(1), 0, &vtProp1, 0, 0);
			if (WBEM_S_NO_ERROR != hr) {
				if (pclsObj) {
					VariantClear(&vtProp1);
					pclsObj->Release(); pclsObj = NULL;
				}
				throw std::exception("Failed to get info from PercentUserTime table");
				break;
			}

			/*hr = pclsObj->Get(queryAttrs.at(2), 0, &vtProp2, 0, 0);
			if (WBEM_S_NO_ERROR != hr) {
				if (pclsObj) {
			...
			cpuUsage += (std::stoi(vtProp.bstrVal));
			//cpuCounts = std::stoi(vtProp1.bstrVal);
			///cpuUsage += std::stoi(vtProp2.bstrVal);
			VariantClear(&vtProp);
			VariantClear(&vtProp1);
			//VariantClear(&vtProp2);
			pclsObj->Release(); pclsObj = NULL;
		}
	return cpuUsage / (logicCores ? logicCores: 1);
}
